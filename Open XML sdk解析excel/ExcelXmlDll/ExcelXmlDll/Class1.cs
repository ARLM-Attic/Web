...
namespace ExcelXmlDll
{
    public interface COMTestIfce
    {
        void ExcelAnalyse(string path, int Testid, string picturepath,string xmlpath,int ID);
    };
    public class ExcelXmlDllClass : COMTestIfce
    {
        ...
        public void ExcelAnalyse(string path, int Testid, string picturepath, string xmlpath, int ID)
        {
            tempTestid = Testid;
            //-----------------tree.xml
            string tree = tempTestid + "-" + ID + "-node.xml";
            ...
            //-----------------attr.xml
            string attrs1 = tempTestid + "-" + ID + "-attr.xml";
            //创建XmlDocument对象xmlDoc  
            ...
            string path1 = xmlpath + tree;
            string path2 = xmlpath + attrs1;
            ...
            using (SpreadsheetDocument document = SpreadsheetDocument.Open(path, false))
            {
                ...
                Console.WriteLine(chineseattr.Count);
                ...
                for (int k = 0; k < work.Count; k++)
                {
                    ...
                    foreach (OpenXmlElement node in nodes)
                    {
                        ...
                        if (node.LocalName == "mergeCells")
                        {
                            string prefix = work[k] + "/" + "合并单元格";
                            Createroot(lol.ToString(), sheetid.ToString(), "合并单元格", prefix, "true", xmlDoc, root);
                            lol++;
                            foreach (OpenXmlElement node1 in node)
                            {
                            ...
                            foreach (Sheet sheet1 in sheets)
                            {
                                ...
                                if (drawingPart1.ImageParts != null)
                                {
                                    ...
                                    foreach (var part in drawingPart1.ImageParts)
                                    {
                                        ...
                                        string name = tempTestid + "_" + work[temp] + "_"+ID+ "_" + i + ".jpg"; 
